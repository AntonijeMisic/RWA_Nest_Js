...
export class WorkLogService {
    ...
    constructor(@Inject('WORKLOG_REPOSITORY')
    ...
    async resumeWork(userId: number): Promise<WorkLog> {
        ...
        const now = new Date();
        const diffMinutes = Math.floor((now.getTime() - new Date(workLog.startBreakTime).getTime()) / 1000 / 60);
    ...
    async getWorkLogsForUser(userId: number): Promise<WorkLog[]> {
        return this.workLogRepo.find({ where: { userId }, order: { workDate: 'DESC' } });
    }
    ...
    async getWorkLogsForWeek(userId: number, referenceDate: Date): Promise<WorkLog[]> {
    ...
    async getWorkLogForDate(userId: number, date: string): Promise<WorkLog | null> {

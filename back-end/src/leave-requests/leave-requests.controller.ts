...
export class LeaveRequestController {
    ...
    async getLeaveRequestsForWeek(@Param('userId') userId: number, @Query('start') weekStart: string, @Query('end') weekEnd: string): Promise<LeaveRequest[]> {
        return this.service.getApprovedLeavesForUserInPeriod(userId, weekStart, weekEnd);
    }

    @Patch('status')
    updateStatus(@Body() dto: UpdateStatusDto) {
        return this.service.updateRequestStatus(dto.requestId, dto.approverId, dto.statusId);
    }

    @Delete(':requestId/:userId')
    deleteRequest(@Param('requestId') requestId: number, @Param('userId') userId: number) {
